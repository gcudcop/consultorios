...
public class FEstudiante {
   ...
   public static ArrayList<Estudiante> obtenerEstudianteDadoCedulaDocente(String cedula) throws Exception {
        ArrayList<Estudiante> lst = new ArrayList<Estudiante>();
        try {
            ArrayList<Parametro> lstP = new ArrayList<Parametro>();
            String sql = "select * from juridico.f_select_estudiante_dado_cedula_docente(?)";
            lstP.add(new Parametro(1, cedula));
            ConjuntoResultado rs = AccesoDatos.ejecutaQuery(sql, lstP);
            lst = llenarEstudiante(rs);
            rs = null;
